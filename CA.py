...
class SSLCertificateGenerator:
	...
	def _create_csr(self, cert_name, key):
		req = crypto.X509Req()
		req.get_subject().emailAddress = cert_name
		req.set_pubkey(key)
		req.sign(key, "sha256")
		return req
	...
	def _sign_csr(self, req, ca_key, ca_cert, cert_org=False, cert_ou=False, usage=3, days=3650, alt_names=[]):
		...
		if cert_ou:
			cert.get_subject().OU = req.get_subject().emailAddress
		else:
			cert.get_subject().OU = req.get_subject().emailAddress
		...
		if usage == 1:
			...
			cert.add_extensions([
				crypto.X509Extension(b"extendedKeyUsage", True, b"emailProtection"),
			])
