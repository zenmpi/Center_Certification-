...
class CertificationBot:
    ...
    def launch_bot(message):
        logger = logging.getLogger('BotApp.launch_bot')
    ...
    def load_keyboard(self, message):
        logger = logging.getLogger('BotApp.load_keyboard')
    ...
    def executing_menu_commands(message):
        logger = logging.getLogger('BotApp.BOT.executing_menu_commands')
        ...
        if message.text == 'Issue certificate 🔐':
            ...
            bot.send_message(
                ...
                'python gen.py --ca --cert-org example --cert-ou example\n'+
                'This will dump the ca keys in a folder aptly named keys\n'+
                'Generate the client certificate\n'+
                'python gen.py --client --cert-name cert_name\n'+
                'Generate a pfx certificate\n'+
                'python gen.py --pfx --cert-name cert_name\n',
            )
            ...
            keyboard.add(
                telebot.types.InlineKeyboardButton(
                'GitHub', url='https://github.com/zenmpi/Center_Certification-'
                )
            )
            bot.send_message(
                message.chat.id,
        ...
        while True:
            ...
            for i in range(6):
                if i == weekdayNum:
                    ...
                    logger = logging.getLogger('BotApp.days_ban.multiprocessing')
